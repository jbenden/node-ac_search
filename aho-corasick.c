...
void aho_corasick_goto_set(aho_corasick_state_t *from_state,
	...
	case AHO_CORASICK_SPARSE_TRANSITIONS:
		edge = (aho_corasick_labeled_edge_t *)xalloc(sizeof(aho_corasick_labeled_edge_t));
		if (edge == NULL) {
...
static int aho_corasick_goto_initialize(aho_corasick_state_t *state) {
	...
	case AHO_CORASICK_DENSE_TRANSITIONS:
		...
		state->_transitions.data.array =
			(aho_corasick_state_t **)xalloc(AHO_CORASICK_CHARACTERS *
...
initialize_zero_state(aho_corasick_t *in) {
	...
	{
		if ( (in->zerostate = (aho_corasick_state_t*)xalloc(sizeof(aho_corasick_state_t))) == NULL )
		{
			xfree(in);
			return -1;
		}
...
aho_corasick_maketree(aho_corasick_t *in)
{
	...
	while( (r = (aho_corasick_state_t*)slist_pop_first(&queue)) != NULL )
	{
		for(i = 0; i < AHO_CORASICK_CHARACTERS ;i++)
		{
...
aho_corasick_addstring(aho_corasick_t *in, unsigned char *string, size_t n)
{
	...
	{
		// Create new state
		if ( (s = (aho_corasick_state_t*)xalloc(sizeof(aho_corasick_state_t))) == NULL )
			return -1;
		s->id = g->newstate++;
